...
def process_section(timestamp, metrics, prefix, section):
    for stat in section:
        if re.match('.*marvel.*', stat):
            continue
        if re.match('.*kibana.*', stat):
            continue
        if re.match('.*primaries.*', stat):
            continue
...
def send_to_graphite(metrics):
    if args.debug:
        ...
        for metrics_i in [metrics[i:i + 50] for i in range(0, len(metrics), 3)]:
            payload = pickle.dumps(metrics_i)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--graphite-port', default=2004, type=int,
                        help='graphite pickle protocol port. Default: %(default)s')
    parser.add_argument('-i', '--interval', default=60, type=int, help='interval in seconds. Default: %(default)s')
    parser.add_argument('--health-level', choices=['cluster', 'indices', 'shards'], default='indices',
                        help='The level of health metrics. Default: %(default)s')
